...
Chadron_list = cfg.Chadron_MC_PDG_ID 
...
def build_decay_tree_from_Bhadron(event, sample_name, b2lnu = True, have_evt_ID=False):
    ...
    if qq  == "bb":
        hadron_list = Bhadron_list
    elif qq == "cc":
        hadron_list = Chadron_list
    else:
        raise ValueError(f"This function only works on Zbb or Zcc events and qq = {qq}")
    ...
    for p in range(particles):
        ...
        if cosTheta>0: #ie. in hemisEmin
            ...
            if abs(ID) in hadron_list: # start with B hadron 
    ...
    for p in range(particles):
        ...
        if cosTheta<0: #ie. in hemisEmax
            ...
            if abs(ID) in hadron_list: # start with B hadron on OS - ensure matter/antimatter if SS is antimatter/matter
...
def build_Zss_decay_tree(event, sample_name, have_evt_ID=False):
    ...
    for p in range(particles):
        ...
        D1 = safe_get(event, "MC_D1", p)
        D2 = safe_get(event, "MC_D2", p)
        D3 = safe_get(event, "MC_D3", p)
        D4 = safe_get(event, "MC_D4", p)
        ...
        #if parent is a s quark
        if abs(M1_ID) in [1, 2, 3, 4, 5] or abs(M2_ID) in [1, 2, 3, 4, 5]:
            #and it itself is not a quark or gluon
            if abs(ID) not in [1,2,3,4,5,21]:
                # and it decays to something
                if any(d != -999 for d in [D1, D2, D3, D4]):

                    #then run parenting algorithm
                    ...
                    if cosTheta>0: #ie. in hemisEmin
                        hemisEmin_particles.append(ID)

                        decay_chain[i_SS] = decay
                        decay_chain_hemis_sgn[i_SS] = hemis_sgn

                        SS_b_parent_IDs = b_parent_IDs
                        SS_b_parent_sgns = b_parent_sgns 

                        i_SS+=1

                    elif cosTheta<0:
                        hemisEmax_particles.append(ID)

                        OS_decay_chain[i_OS] = decay
                        OS_decay_chain_hemis_sgn[i_OS] = hemis_sgn

                        OS_b_parent_IDs = b_parent_IDs
                        OS_b_parent_sgns = b_parent_sgns 

                        i_OS+=1

                    else:
                        raise ValueError(f"cosTheta = {cosTheta} which isn't > or < 0")
                
                else: # all daughters are -999 ie. stable

                    if cosTheta>0: #ie. in hemisEmin
                        hemisEmin_stable_particles.append(ID)


                    elif cosTheta<0:
                        hemisEmax_stable_particles.append(ID)
    ...
    # Convert PDG codes to particle names
    for i in range(i_SS):
        named_decay[i] =  map_nested(pdg_name, decay_chain[i])
    
    for i in range(i_OS):
        OS_named_decay[i] =  map_nested(pdg_name, OS_decay_chain[i])
    ...
    return {
        "evt_id":evt_ID,
        #**{f"decay_chain{i}": decay_chain[i] for i in range(i_SS)},
        **{f"named_decay{i}": named_decay[i] for i in range(i_SS)},
        **{f"decay_chain_hemis_sgn{i}": decay_chain_hemis_sgn[i] for i in range(i_SS)},
        "SS_b_parent": map_nested(pdg_name, SS_b_parent_IDs),
        "SS_b_parent_hemis_sgn": SS_b_parent_sgns,
        "SS_unstable_MC_particles_from_s": map_nested(pdg_name, hemisEmin_particles),
        "SS_stable_MC_particles_from_s": map_nested(pdg_name, hemisEmin_stable_particles),
        #**{f"OS_decay_chain{i}": OS_decay_chain[i] for i in range(i_OS)},
        **{f"OS_named_decay{i}": OS_named_decay[i] for i in range(i_OS)},
        **{f"OS_decay_chain_hemis_sgn{i}": OS_decay_chain_hemis_sgn[i] for i in range(i_OS)},
        "OS_q_parent": map_nested(pdg_name, OS_b_parent_IDs),
        "OS_q_parent_hemis_sgn": OS_b_parent_sgns,
        "OS_unstable_MC_particles_from_s": map_nested(pdg_name, hemisEmax_particles),
        "OS_stable_MC_particles_from_s": map_nested(pdg_name, hemisEmax_stable_particles),
        "SS_reco_final_states": SS_fs,
        "OS_reco_final_states": OS_fs,
        "SS_MC_final_states": fs_SS_particles,
        "OS_MC_final_states": fs_OS_particles,
        "SS_missed_particles": missed_particles,
        "OS_missed_particles": OS_missed_particles,
...
if __name__ == "__main__":
    ...
    samples = ["p8_ee_Zcc_ecm91"]#cfg.sample_allocations["combined_signal"]#["p8_ee_Zbb_ecm91"]#cfg.sample_allocations["hadronic_background"]#["p8_ee_Zbb_ecm91"]#cfg.sample_allocations["Bc2lnu_background"] #["p8_ee_Zbb_ecm91_EvtGen_Bu2MuNu"]##["p8_ee_Zbb_ecm91_EvtGen_Bu2TauNuTAUHADNU", "p8_ee_Zbb_ecm91_EvtGen_Bu2MuNu"] #["p8_ee_Zbb_ecm91_EvtGen_Bu2TauNuTau2MuNuNu"]#cfg.sample_allocations["Bu2lnu_background"]
    runmode = "process_with_MC_full_prelim" #"Bc2lnu_background_no_lepton_veto" #"Bu2lnu_background_no_lepton_veto"##"Bu2lnu_background_no_lepton_veto"
    bdtcut=0.99965
    ...
    for decay in samples:
        ...
        # Collect all fieldnames from all events
        fieldnames = set()
        for r in results:
        ...
        # Save all to CSV
        with open(os.path.join(decay_inputpath,f"Zss_background_analysis_{decay}_bdtcut{remove_dot_etc(str(bdtcut))}.csv"), "w", newline="") as f:
            writer = csv.DictWriter(f, fieldnames=fieldnames)
            writer.writeheader()
            writer.writerows(results)

        print(f"CSV file 'Zss_background_analysis_{decay}_bdtcut{remove_dot_etc(str(bdtcut))}.csv' created successfully!")
