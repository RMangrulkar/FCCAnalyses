# Train bdtl using prelimcuts files
...
#################################
print(f"{30*'-'}")
print(f"CREATING AND SAVING df FOR BDTl TRAINING")
print(f"{30*'-'}\n")


#path to data and outputs
inputpath    = check_inputpath(cfg.bdtl_opts['inputPath']) 
outputpath   = set_outputpath(cfg.bdtl_opts['outputPath'])
yamlpath     = check_inputpath(cfg.fccana_opts['yamlPath'])

#Getting BDT vars for training from yaml
bdtvars_list = cfg.bdtl_opts['mvaBranchList']
...
signal_decays =  cfg.bdtl_opts["signalAllocation"]
background_decays =  cfg.bdtl_opts["backgroundAllocation"]
...
#Check4:Ratio sum of weights in ss:ud = bb_eff*BF(Z->bb)/cc_eff*BF(Z->cc)
...
df.to_pickle(os.path.join(outputpath,'bdtl_dataframe.pkl'))
print("DataFrame saved successfully!")

# Write key info about df to log file
with open(os.path.join(outputpath,'bdtl_dataframe_info.log'), 'a') as log_file:
